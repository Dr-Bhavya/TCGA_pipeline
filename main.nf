...
workflow {
    ...
    main:
    // Create input channel from a CSV file listing input manifest file paths
    read_manifest_ch = channel.fromPath(params.input_manifest)
            .splitCsv(header: true)
            .map {
                    row -> tuple(row.cancer_type, file(row.cancer_manifest_file_path), file(row.normal_manifest_file_path)) 
                    }

    read_metadata_ch = channel.fromPath(params.input_metadata)
            .splitCsv(header: true)
            .map {
                    row -> tuple(row.cancer_type, file(row.metadata_file_path)) 
                    }
...
output {
    ...
    pdf_rra_up       { path 'Upregulated_DEGs' }
    tiff_rra_up      { path 'Upregulated_DEGs' }   
    pdf_rra_dn       { path 'Downregulated_DEGs' }   
    tiff_rra_dn      { path 'Downregulated_DEGs' }   
}